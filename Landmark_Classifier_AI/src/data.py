...
def get_data_loaders(
    batch_size: int = 32, valid_size: float = 0.2, num_workers: int = 2, limit: int = -1
):
    """
    ...
    data_transforms = {
        "train": transforms.Compose([
            ...
            #transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2),  # Additional augmentation
            #transforms.RandomRotation(20),  # Additional augmentation
            transforms.ToTensor(),
    ...
    n_tot = len(full_train_data)
    indices = torch.randperm(n_tot)

    # If needed, limit the number of data points to consider
    if limit > 0:
